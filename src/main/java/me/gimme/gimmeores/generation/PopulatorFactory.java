...
public abstract class PopulatorFactory {
    ...
    public static Populator createPopulator(
        ...
        switch (populatorType) {
            default:
            case VEIN:
                return new VeinPopulator(plugin, loadedChunksData, material, size, triesPerChunk, minHeight, maxHeight,
                        replaceWith, canReplace, worlds, biomes);
            case SINGLE:
                return new SinglePopulator(plugin, loadedChunksData, material, size, triesPerChunk, minHeight, maxHeight,
                        replaceWith, canReplace, worlds, biomes);
        }
    }
}
